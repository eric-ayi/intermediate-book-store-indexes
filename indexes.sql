...
--Analyze performance of selecting customer_ids with orders more than 18 after adding partial index
EXPLAIN ANALYZE
SELECT
    customer_id,
    quantity
...
--analyze selecting selecting customers with id greater than 500 after indexing customer_id
EXPLAIN ANALYZE
SELECT
    *
FROM
...
--check if cluster worked. Query expected to return customer in order of increasing customer_id
SELECT
    *
FROM
    customers
...
--Examine indexes already present in orders table after creating multicolumn index
SELECT
    *
FROM
    pg_Indexes
...
--Examine indexes already present in orders table after dropping multicolumn index
SELECT
    *
FROM
    pg_Indexes
...
--Recreate multicolumn index with one additional column
